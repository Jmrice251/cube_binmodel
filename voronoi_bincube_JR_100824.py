...
# output spectrum as image file
output_spec_fn = bin_info_obj['output_spec_fn'] if 'output_spec_fn' in bin_info_obj else None


# ----------- READ CUBE & REGION ---------------------------------------------------------------------------------------------
print("Reading cube...")
cube = Cube(cube_fn)     # do a copy here to fully load into memory, important for later
...
lo_pix_wave = int(np.floor((cube.wave.get_start() - CRVAL) / CDELT))
hi_pix = int(np.ceil((range_max - CRVAL) / CDELT))
hi_pix_wave = int(np.ceil((cube.wave.get_end() - CRVAL) / CDELT))
x0=np.arange(hi_pix_wave + 1 - lo_pix_wave)
wave_A=((x0+lo_pix_wave)*CDELT + CRVAL)

# sum over range, along the wave axis
sum_image = cube[lo_pix:(hi_pix+1), :, :].sum(axis=0)

if output_cube_fn or output_spec_fn:
    # get cube data necessary to write binned cube and spectrum
    cube_data = cube.data.data
    cube_var = cube.var.data
    cube_wcs = cube.wcs
    cube_wave = cube.wave
    cube_unit = cube.unit
    zeroed_cube_data = cube_data
    zeroed_cube_data[np.isnan(cube_data)] = 0
    zeroed_cube_var = cube_var
    zeroed_cube_var[np.isnan(cube_var)] = 0.000001
    del cube_data, cube_var
    gc.collect()
...
if regions_fn == "None":
    ...
    while i < len(noises):
        if noises[i] == 0 or signals[i] == np.nan:
            noises.pop(i)
            signals.pop(i)
            x_inside.pop(i)
            y_inside.pop(i)
...
# Voronoi binning
bin_number, _, _, _, _, sn, nPixels, _ = voronoi_2d_binning(
...
if output_cube_fn:
    ...
    for i in tqdm(range(len(inside_pix_coords))):
        bin_index, coord = bin_number[i], inside_pix_coords[i]
        avg_sig_array[bin_index] += zeroed_cube_data[:, coord[0], coord[1]]
        avg_var_array[bin_index] += zeroed_cube_var[:, coord[0], coord[1]]

    reshaped_nPixels = nPixels.reshape((n_bins,1))
    for bin_index in tqdm(range(n_bins)):
        avg_sig_array[bin_index] = avg_sig_array[bin_index] / (reshaped_nPixels[bin_index])             # signal for each bin
        avg_var_array[bin_index] = avg_var_array[bin_index] / (reshaped_nPixels[bin_index]**2)      # variance for each bin 
...
if output_spec_fn:
    ...
    for i in tqdm(range(len(inside_pix_coords))):
        bin_index, coord = bin_number[i], inside_pix_coords[i]
        if 2.25*target_sn >= sn[bin_index] >= target_sn:
            avg_sig_array[bin_index] += zeroed_cube_data[:, coord[0], coord[1]]
            avg_var_array[bin_index] += zeroed_cube_var[:, coord[0], coord[1]]
    ...
    plt.figure(figsize=(17, 5)) # plot the spectrum
    plt.title('Average Spectrum For High S/N Bins')  
    ...
    plt.savefig(output_spec_fn, bbox_inches='tight')
