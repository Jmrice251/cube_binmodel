...
for coord in tqdm(inside_pix_coords):
    ...
    result_map[coord] = fitresult_dict[bin_id]['final_params']['A2'].value
    
# export map images
# velo_map = result_map[:, :, 0]*299792.458
A2_map = result_map
# A3_map = 3.0*result_map[:, :, 2]
# sigma_map = result_map[:, :, 6]
header = WCS.to_header(cube_wcs.celestial)

# write to files
fits.writeto(filename=f'{result_dir_name}/A2_map.fits', data=A2_map, header=header, overwrite=True)
