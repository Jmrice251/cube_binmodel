...
if output_cube_fn:
    ...
    for i in tqdm(range(len(inside_pix_coords))):
        bin_index, coord = bin_number[i], inside_pix_coords[i]
        # bin_array[coord] = bin_index
        if cube_data[:, coord[0], coord[1]].all() != np.nan:
            avg_signal_array[bin_index] += cube_data[:, coord[0], coord[1]]
        if cube_var[:, coord[0], coord[1]].all() != np.nan:
...
if output_fits_fn:
    # assign each bin number a new random number between 0 and 1
    hc_bin_number = bin_number
    rng = np.random.default_rng()
    rand_bins = rng.integers(low=0, high=10, size=n_bins)
    for i in range(len(inside_pix_coords)):
        hc_bin_number[i] = rand_bins[bin_number[i]]
    
    # create a new bin array with the randomized bin numbers
    hc_bin_array = np.full((N_y,N_x), np.nan, dtype=float)
    for i in range(len(inside_pix_coords)):
        hc_bin_index, coord = hc_bin_number[i], inside_pix_coords[i]
        hc_bin_array[coord] = hc_bin_index

    # write the high contrast bin array to a fits file
    bins_fits = fits.PrimaryHDU(data=hc_bin_array, header=fits.Header(wcs_header))
