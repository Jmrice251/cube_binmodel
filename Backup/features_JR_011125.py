        ...
        def __init__(self, *args, **kwargs):
            ...
            for param in fit_model.parsed_init_params:
                ...
                if param == 'A2':
                    ...
                    self.set_param_hint(name=param, min=(1.e-5)*fit_model.parsed_init_params[param].value, max=2*fit_model.parsed_init_params[param].value)
                else:
                    self.set_param_hint(name=param, min=fit_model.parsed_init_params[param].min, max=fit_model.parsed_init_params[param].max)
            init_vals = {}    
            for param in fit_model.parsed_init_params:
                if param == 'z' or 'sigma' in param:
...
def fit_lm(label, fit_model, X, Y, dY, method_name, contpoly_order):
    ...
    try:
        ...
        return (label, result.params, contpoly_coefs, isCovarCalculated)
