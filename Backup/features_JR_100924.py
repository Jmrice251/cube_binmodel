    ...
    class ParsedModelLMFIT(Model):       # define a LMFIT model class
        ...
        def __init__(self, *args, **kwargs):
            ...
            for param in fit_model.parsed_init_params:
                ...
                if param != 'z' and param != 'sigma':
                    self.set_param_hint(name=param, min=0)
                    self.set_param_hint(name=param, max=1.2*max(Y))
                elif param == 'z':
                    self.set_param_hint(name=param, min=(-10)*fit_model.parsed_init_params[param].value)
                    self.set_param_hint(name=param, max=(10)*fit_model.parsed_init_params[param].value)
                else:
                    self.set_param_hint(name=param, min=fit_model.parsed_init_params[param].min)
                    self.set_param_hint(name=param, max=fit_model.parsed_init_params[param].max)
            init_vals = {}    
            for param in fit_model.parsed_init_params:
                if param == 'z' or param == 'sigma':
                    init_vals[param] = fit_model.parsed_init_params[param].value
                else:
                    init_vals[param] = 0.8*max(Y)
...
def fit_lm(label, fit_model, X, Y, dY, method_name, contpoly_order):
    ...
    try:
        ...
        # if not then guess#
            ...
            new_model = guess_model(fit_model, Y - contsub)       
