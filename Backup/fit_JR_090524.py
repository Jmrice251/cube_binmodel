...
for res in tqdm(results):
    ...
    if res[1] is not None:
        ...
        if (max(Amps) != Amps[0] and max(Amps) != Amps[1]) or min([Amps[x] for x in [0,1]]) < 0:
            res[1]['A1'].value, res[1]['A2'].value, res[1]['A4'].value, res[1]['A5'].value, res[1]['z'].value, res[1]['sigma'].value = 0, 0, 0, 0, redshift, 0
                ...
                if user_input == 'y' or user_input == 'Y':
                    ...
                    for bin_id in tqdm(range(n_bins)):  # add the signal and variance of high sigma bins
                        if fitresult_dict[bin_id]['final_params'] is not None:
                            if fitresult_dict[bin_id]['final_params']['sigma'].value >= 1:
