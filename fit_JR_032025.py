...
constrain = fit_options_obj['constrain'] if 'constrain' in fit_options_obj else None
...
# Do work in parallel
    print('Fitting...')
    results = Parallel(n_jobs=numprocs, backend="loky")(delayed(features.fit_lm)(bin, Model, wave_A, avg_signal_array[bin], avg_noise_array[bin], method, constrain, model_fn, redshift, sn_cutoff, contpoly_order=contpoly_order) for bin in tqdm(range(n_bins)))
...
while True:
                ...
                if user_input == 'y' or user_input == 'Y':
                    ...
                    for bin_id in tqdm(range(n_bins)):  # add the signal and variance of high sigma bins
                        if fitresult_dict[bin_id]['final_params'] and fitresult_dict[bin_id]['final_params']['A2'].stderr:
                            if np.abs(fitresult_dict[bin_id]['final_params']['A2'].value/fitresult_dict[bin_id]['final_params']['A2'].stderr) > sn_cutoff:
    ...
    if len(split_list) == 2:
        ...
        try:
            ...
            if isinstance(xpixel,int):
                array_coord = (ypixel, xpixel)
                pix_coord = (ypixel + 1, xpixel + 1)
            else:
                array_coord = (ypixel[0], xpixel[0])
                pix_coord = (ypixel[0] + 1, xpixel[0] + 1)
        except:
    ...
    try:
        pix_bin_num = new_bin_array[array_coord]
        if pix_bin_num < 0:
    ...
    sky_coord_str = pixel_to_skycoord(array_coord[1], array_coord[0], cube_wcs).to_string()
    print(f'RA DEC (in degrees): {sky_coord_str}')
    print(f'Bin number: {old_bin_array[array_coord]}')
    
    if final_params is None:               # bin is not fitted
        print(f'Bin {old_bin_array[array_coord]} is not fitted')
